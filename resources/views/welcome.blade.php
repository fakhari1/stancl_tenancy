...
<div
    ...
    @if (Route::has('login'))
        <div class="hidden fixed top-0 right-0 px-6 py-4 sm:block">
            <span class="text-sm text-gray-700 dark:text-gray-500" style="margin-right: 1.5rem; user-select: none;">
                Your Tenant is {{ tenant('id') ?? 'Central' }}
            </span>
            @auth
                <a href="{{ url('/dashboard') }}"
                   class="text-sm text-gray-700 dark:text-gray-500 underline">Dashboard</a>
            @else
                <a href="{{ tenant('id') ?? '' }}/login" class="text-sm text-gray-700 dark:text-gray-500 underline">Log
                    in</a>

                @if (Route::has('register'))
                    <a href="{{ tenant('id') ?? '' }}/register"
